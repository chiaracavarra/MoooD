...
void libusb_cleanup(struct libusb_device_handle *handle) {
	libusb_close(handle);
	libusb_exit(NULL);
}
...
static void print_data(pkt_t *pkt)
{
	...
	fprintf(stderr, "R  : %d\n", le32toh(pkt->rgb_data.red_dc));
	fprintf(stderr, "G  : %d\n", le32toh(pkt->rgb_data.green_dc));
	fprintf(stderr, "B  : %d\n", le32toh(pkt->rgb_data.blue_dc));
}


int main(int argc, char **argv) {
	signal(SIGINT, sighandler);  // save time and avoid sigaction
	...
	struct libusb_device_handle *handle = libusb_open_device_with_vid_pid(NULL, USB_VENDOR_ID, USB_PRODUCT_ID);
	if (handle == NULL) {
	...
	if (conf_desc->bNumInterfaces == 1) {
		//		const struct libusb_interface_descriptor *iface = conf_desc->interface->altsetting;
		//		const struct libusb_endpoint_descriptor *ep = iface->endpoint;
		//
		//		for (int i = 0; i < iface->bNumEndpoints; ep++) {
	...
	while ((c = getopt (argc, argv, "nfs:")) != -1)
	{
		...
		case 's': // set color
		{
			...
			unsigned int red_dc;
			unsigned int green_dc;
			unsigned int blue_dc;

			pkt.header.len = sizeof(data_pkt_t);

			if (sscanf(optarg, "r%d,g%d,b%d", &red_dc, &green_dc, &blue_dc) == 3)
			{
				pkt.rgb_data.red_dc = htole32(red_dc);
				pkt.rgb_data.green_dc = htole32(green_dc);
				pkt.rgb_data.blue_dc = htole32(blue_dc);
	...
	pthread_create(&rxThread, NULL, check_upload_status, handle);
	pthread_create(&txThread, NULL, upload_firmware, handle);
